...
from refusal_scorer import RefusalScorerRegistry


def run_refusal_scorer(
    input_path: str,
    output_path: str,
    scorer_name: str = "exact-match",
    batch_size: int = 16,
):
    scorer_class = RefusalScorerRegistry.get(scorer_name)
    if scorer_class is None:
        raise KeyError(
            f"RefusalScorer '{scorer_name}' is not registered. Available: {list(RefusalScorerRegistry._scorers.keys())}"
        )
    ...
    if os.path.isfile(input_path) and os.path.isfile(output_path):
        scorer.compute_refusal_scores(input_path, output_path)
    else:
        if not os.path.exists(output_path):
            os.makedirs(output_path, exist_ok=True)
        for input_file in glob.glob(os.path.join(input_path, "*.jsonl")):
            if not input_file.endswith("_refusal_scores.jsonl"):
                ...
                if not os.path.exists(output_filename) or (
                    ...
                    scorer.compute_refusal_scores(input_file, output_filename)


if __name__ == "__main__":
    fire.Fire(run_refusal_scorer)
