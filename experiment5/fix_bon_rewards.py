...
def main(
    ...
    It looks at each json blob's `watermarked_texts_reward_score` field
    This field should be a list of floats with length >= N

    It picks the maximum value from the first N values in the list
    and writes it to the `watermarked_text_reward_score` field
    ...
    for input_file in input_files:
        ...
        for item in data:
            reward_scores = item["watermarked_texts_reward_score"][:n]
            max_reward = max(reward_scores)
            max_idx = reward_scores.index(max_reward)
            ...
            for plural_field, singular_field in FIELD_MAPPINGS.items():
                if plural_field in item:
                    if plural_field == "watermarked_texts_reward_score":
                        # For the reward_score field, use the max value
                        item[singular_field] = max_reward
                    else:
                        # For all other fields, use the value at max_idx
                        item[singular_field] = item[plural_field][max_idx]

        # Remove the _rewards.jsonl suffix and change to .jsonl for the plotting to work correctly
        output_file = os.path.join(
            output_folder, filename.replace("_rewards.jsonl", ".jsonl")
        )
